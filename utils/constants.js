const overAllData = {
  onGoingProjects: [
    {
      slug: "prakriti",
      name: "PRAKRITI",
      image: "assets/images/lml-map-img.jpg",
      location: "Tambaram",
      floor: "2BHK | 3BHK",
      price: "72 Lakhs",
      locationInfo: {
        lat: 12.9249,
        lng: 80.1,
      },
      detail: {
        bannerImage: "bg-home-1.jpeg",
        logo: "logo-1.svg",
        heading: "LML PRAKRITI, TAMBARAM",
        keyLocation: "",
        detailDescription:
          "Prakriti in Sanskrit means 'Nature'. It is also what is found in nature in its unaltered form and everything that you can sense. Prakriti is in your thoughts, memories, desires, and even in your aspirations. Bringing Prakriti into your life is LML HOMES, which is an embodiment of one of the finest",
        offer: {
          price: "72 Lakhs",
          info: "2,3 & 4 BHK",
          subContent: "Excluding Registration",
          subContentNxtLine: " (Ready To Occupy)",
        },
        pdf: "assets/pdfs/lml-prakriti-magazine.pdf",
        enquire: {
          whatsApp: "+918898989898",
          phone: "+918898989898",
        },
        bannerHighlights: [
          {
            icon: "",
            label: "2,3&4 BHK",
          },
          {
            icon: "",
            label: "GATED COMMUNITY OF 231 UNITS",
          },
          {
            icon: "",
            label: "READY TO OCCUPY",
          },
          {
            icon: "",
            label: "RERA TN/01/BUILDING/0147/2019",
          },
          {
            icon: "credaui-white-new.png",
            label: "",
          },
        ],
        descriptionHighlights: [
          {
            icon: "proj-high-2.svg",
            label: "2,3&4 BHK",
          },
          {
            icon: "proj-high-5.svg",
            label: "Gated Community",
          },
          {
            icon: "proj-high-4.svg",
            label: "6 Blocks",
          },
          {
            icon: "proj-high-3.svg",
            label: "Area 1024 - 1395 sq.ft",
          },
          {
            icon: "proj-high-6.svg",
            label: "RERA & CMDA Approved",
          },
          {
            icon: "proj-high-2.svg",
            label: "Stilt + 4 Floors",
          },
        ],
        walkthrough: {
          video: "https://www.youtube.com/embed/rdaD3bhM6os?si=yeaZ3WIXUZxZNRSU",
          description:
            "At LML Prakriti, every day offers a new opportunity to immerse yourself in an extravagant lifestyle.",
          question: "WHY LML PRAKRITI?",
          answer:
            " Naturally set in an enchanting destination and advantageously located just 2 kilometers away from the bustling Tambaram Railway Station and 5-minute drive from the Tambaram Main Bus Terminus., Travelling to different parts of the city from LML Prakriti gets easier and faster to the double connectivity by rail and by road too.",
        },
        amenities: {
          description:
            "At Prakriti, we welcome you to a world of state-of-the-art amenities and facilities that help you savour a premium lifestyle.",
          amenityList: [
            {
              image: "assets/prakriti/gym.png",
              label: "GYM",
            },
            {
              image: "assets/prakriti/cctv.png",
              label: "CCTV Surviellance",
            },
            {
              image: "assets/prakriti/Department-Store.png",
              label: "Departmental Store",
            },
            {
              image: "assets/prakriti/Kids-Play-Area.png",
              label: "Kids Play Area",
            },
            {
              image: "assets/prakriti/lift.png",
              label: "Elevator",
            },
            {
              image: "assets/prakriti/Mini-Hall.png",
              label: "Multi-Purpose Hall",
            },
            {
              image: "assets/prakriti/Mini-Theatre.png",
              label: "Mini Theatre",
            },
            {
              image: "assets/prakriti/Parking.png",
              label: "Parking",
            },
            {
              image: "assets/prakriti/security.png",
              label: "24x7 Security",
            },
            {
              image: "assets/prakriti/Sewage-Treatment-Plant.png",
              label: "Sewage Treatment Plant",
            },
            {
              image: "assets/prakriti/Temple.png",
              label: "Temple",
            },
            {
              image: "assets/prakriti/Walking-Path-way.png",
              label: "Walking-Path-way",
            },
            {
              image: "assets/prakriti/guest-bedroom.png",
              label: "Guest Bedroom",
            },
          ],
        },
        locationInfo: {
          heading: "Location - 'Tambaram' ",
          description:
            "This gated community apartments is the perfect place to find a home that's conveniently close to all the key infrastructures you need. With just a 5-minute drive, you can reach college, hospitals, malls and more.",
          CenterLocation: {
            lat: 12.932279624083709,
            lng: 80.10022599362797,
          },
          centerLocationMarker: "logo-1.svg",
          centerLocationLink: "https://goo.gl/maps/oRJzK5tR4zB8Ved69",
          locations: [
            {
              name: "Commute",
              icon: "commute.svg",
              locationList: [
                {
                  name: "Laxmipuram bus stop - 500 Mts",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Tambaram bus terminus - 2.25km",
                  location: {
                    lat: 12.93755685765274,
                    lng: 80.12783534572243,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Irumbuliyur Junction - 2.25km",
                  location: {
                    lat: 12.916360460350832,
                    lng: 80.10246283668842,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Perungalathur bus stop - 3km",
                  location: {
                    lat: 12.907708224311769,
                    lng: 80.09600113213342,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Camp road Junction - 6km",
                  location: {
                    lat: 12.923500653471326,
                    lng: 80.14269379686989,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 12.93190729151106,
                    lng: 80.10032021762002,
                    markerIcon: "logo-1.svg",
                  },
                },
              ],
            },
            {
              name: "Education",
              icon: "education.svg",
              locationList: [
                {
                  name: "Sri Ramakrishna Matriculation Higher Secondary School - 550Mts",
                  location: {
                    lat: 12.92444276299563,
                    lng: 80.10183249502863,
                    markerIcon: "education-fill.svg",
                  },
                },

                {
                  name: "San Academy - 500Mts",
                  location: {
                    lat: 12.93669876846475,
                    lng: 80.09686675484583,
                    markerIcon: "education-fill.svg",
                  },
                },

                {
                  name: "Sri Ramana Vidyalaya Matriculation School - 700Mts",
                  location: {
                    lat: 12.926293947622323,
                    lng: 80.10877231850334,
                    markerIcon: "education-fill.svg",
                  },
                },

                {
                  name: "Lords international school - 2km",
                  location: {
                    lat: 12.945575039894583,
                    lng: 80.09726731231423,
                    markerIcon: "education-fill.svg",
                  },
                },

                {
                  name: "Corley Higher Secondary School - 2.5km",
                  location: {
                    lat: 12.921332346616703,
                    lng: 80.11691151707835,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Seventh day Adventist High School - 2.7km",
                  location: {
                    lat: 12.910565407723862,
                    lng: 80.10075043701059,
                    markerIcon: "education-fill.svg",
                  },
                },

                {
                  name: "SKB Vidhyashram school - 3.2km",
                  location: {
                    lat: 12.925728312057148,
                    lng: 80.07923477893885,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 12.93190729151106,
                    lng: 80.10032021762002,
                    markerIcon: "logo-1.svg",
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "AG Hospital - 1.5km",
                  location: {
                    lat: 12.92561101472616,
                    lng: 80.11343047547665,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Hindu Mission Hospital - 1.5km",
                  location: {
                    lat: 12.923872416997238,
                    lng: 80.11406425717608,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Kumaran Hospital - 1.5km",
                  location: {
                    lat: 12.928072179486508,
                    lng: 80.11226713034051,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Dr.Agarwal Eye Hospital - 1.5km",
                  location: {
                    lat: 12.929943017865357,
                    lng: 80.12042956271776,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Deepam Hospital - 2.7km",
                  location: {
                    lat: 12.929584795087326,
                    lng: 80.11774828246507,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Annai Arul Hospital - 2.8km",
                  location: {
                    lat: 12.922239311591559,
                    lng: 80.09231232689889,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Cosh Hospital - 3.6km",
                  location: {
                    lat: 12.921621586384552,
                    lng: 80.12797386900657,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Parvathy Hospital - 6km",
                  location: {
                    lat: 12.951255276113413,
                    lng: 80.1440796166696,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Shree Balaji Hospital - 6.5km",
                  location: {
                    lat: 12.953759917118951,
                    lng: 80.13881270691608,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 12.93190729151106,
                    lng: 80.10032021762002,
                    markerIcon: "logo-1.svg",
                  },
                },
              ],
            },
            {
              name: "Retail",
              icon: "retail.svg",
              locationList: [
                {
                  name: "Supermarket - 600m",
                  location: {
                    lat: 12.951332878176972,
                    lng: 80.13919729819503,
                    markerIcon: "retail-fill.svg",
                  },
                },
                {
                  name: "Tambaram main market - 2.2km",
                  location: {
                    lat: 12.922401897367735,
                    lng: 80.10942122508587,
                    markerIcon: "retail-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 12.93190729151106,
                    lng: 80.10032021762002,
                    markerIcon: "logo-1.svg",
                  },
                },
              ],
            },
            {
              name: "Commercial Hubs",
              icon: "commercial-hub.svg",
              locationList: [
                {
                  name: "Vidhya Theatre - 1.8km",
                  location: {
                    lat: 12.926298971551374,
                    lng: 80.11164287294518,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "MR Theatre - 2.2km",
                  location: {
                    lat: 12.925031627823394,
                    lng: 80.1163336677995,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "National Theatre - 2.3km",
                  location: {
                    lat: 12.929353892464878,
                    lng: 80.11522081483487,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "Varadharaja Theatre - 5km",
                  location: {
                    lat: 12.937784092595129,
                    lng: 80.13901830812283,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "Vettri Theatre - 6.5km",
                  location: {
                    lat: 12.954793622119125,
                    lng: 80.14213757027292,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 12.93190729151106,
                    lng: 80.10032021762002,
                    markerIcon: "logo-1.svg",
                  },
                },
              ],
            },
          ],
          address: "RajMohan Nagar, Tambaram, Chennai - 600 045",
        },
        layout: {
          heading: "SITE PLAN [only few units left]",
          // description:
          //   "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "Key Plan",
              image: "/assets/prakriti/prakriti-keyPlan.jpg",
            },
            {
              name: "A Block",
              image: "/assets/prakriti/layouts/A-Block.png",
              component: "A_Block",
            },
            {
              name: "B Block",
              image: "/assets/prakriti/layouts/B-Block.png",
              component: "B_Block",
            },
            {
              name: "C Block",
              image: "/assets/prakriti/layouts/C-Block.png",
              component: "C_Block",
            },
            {
              name: "D Block",
              image: "/assets/prakriti/layouts/D-Block.png",
              component: "D_Block",
            },
            {
              name: "E Block",
              image: "/assets/prakriti/layouts/E-Block.png",
              component: "E_Block",
            },
            {
              name: "F Block",
              image: "/assets/prakriti/layouts/F-Block.png",
              component: "F_Block",
            },
          ],
        },
        unitPlan: {
          heading: "Unit Plans",
          // description:
          //   "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1004 SQFT",
                  image: "/assets/prakriti/B_406.jpg",
                },
                {
                  name: "1008 SQFT",
                  image: "/assets/prakriti/C_408.jpg",
                },
                {
                  name: "1013 SQFT",
                  image: "/assets/prakriti/B_403.jpg",
                },
                {
                  name: "1014 SQFT",
                  image: "/assets/prakriti/B_405.jpg",
                },
                {
                  name: "1019 SQFT",
                  image: "/assets/prakriti/B_410.jpg",
                },
                {
                  name: "1023 SQFT",
                  image: "/assets/prakriti/C_309.jpg",
                },
                {
                  name: "1023 SQFT",
                  image: "/assets/prakriti/C_409.jpg",
                },
                {
                  name: "1024 SQFT",
                  image: "/assets/prakriti/C_304.jpg",
                },
                {
                  name: "1024 SQFT",
                  image: "/assets/prakriti/C_404.jpg",
                },
                {
                  name: "1027 SQFT",
                  image: "/assets/prakriti/C_405.jpg",
                },
                {
                  name: "1044 SQFT",
                  image: "/assets/prakriti/C_402.jpg",
                },
              ],
            },
            {
              name: "3BHK",
              sizeList: [
                {
                  name: "1272 SQFT",
                  image: "/assets/prakriti/C_406.jpg",
                },
                {
                  name: "1310 SQFT",
                  image: "/assets/prakriti/C_411.jpg",
                },
                {
                  name: "1362 SQFT",
                  image: "/assets/prakriti/B_409.jpg",
                },

                {
                  name: "1362 SQFT",
                  image: "/assets/prakriti/B_409.jpg",
                },
                {
                  name: "1395 SQFT",
                  image: "/assets/prakriti/B_413.jpg",
                },
              ],
            },
          ],
        },
        subBannerImage: "/prakriti/arshiya-sub-banner.png",
        gallery: {
          heading: "Glimpses of life at prakarthi",
          //subHeading:
          //"With beautiful and thoughtfully-designed homes at Tambaram, you can experience the sunny side of life.",
          list: [
            {
              name: "Handing Over Images",
              images: [
                {
                  image: "prakriti/handling-over-images/handling-over-3.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-4.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-8.jpg",
                  label: "Handing Over Image",
                },

                {
                  image: "prakriti/handling-over-images/handling-over-11.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-15.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-17.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-19.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-23.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-24.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-25.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-26.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-28.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-29.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-30.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-32.jpg",
                  label: "Handing Over Image",
                },
                {
                  image: "prakriti/handling-over-images/handling-over-33.jpg",
                  label: "Handing Over Image",
                },
              ],
            },
            {
              name: "Amenities",
              images: [
                {
                  image: "prakriti/interior/interior-1.jpg",
                  label: "Interior View",
                },
                {
                  image: "prakriti/interior/interior-2.jpg",
                  label: "Interior View",
                },
                {
                  image: "prakriti/interior/interior-3.jpg",
                  label: "Interior View",
                },
                {
                  image: "prakriti/interior/interior-4.jpg",
                  label: "Interior View",
                },
                {
                  image: "prakriti/interior/interior-5.jpg",
                  label: "Interior View",
                },
                {
                  image: "prakriti/interior/interior-6.jpg",
                  label: "Interior View",
                },
              ],
            },
            {
              name: "Live Photos",
              images: [
                {
                  image: "prakriti/exterior/exterior-1.jpg",
                  label: "Exterior View",
                },
                {
                  image: "prakriti/exterior/exterior-2.jpg",
                  label: "Exterior View",
                },
                {
                  image: "prakriti/exterior/exterior-3.jpg",
                  label: "Exterior View",
                },
                {
                  image: "prakriti/exterior/exterior-4.jpg",
                  label: "Exterior View",
                },
              ],
            },
            // {
            //     "name": "Actual Images",
            //     "images": [
            //         {
            //             "image": "prakriti/actual-images/1.jpg",
            //             "label": "Actual Images"
            //         },
            //         {
            //             "image": "prakriti/actual-images/2.jpg",
            //             "label": "Actual Images"
            //         },
            //         {
            //             "image": "prakriti/actual-images/3.jpg",
            //             "label": "Actual Images"
            //         },
            //         {
            //             "image": "prakriti/actual-images/4.jpg",
            //             "label": "Actual Images"
            //         },
            //     ]
            // }
          ],
        },
        testimonial: [
          {
            name: "Mr. Mohandass",
            location: "Perungudi",
            property: "LML Atita",
            description:
              "We own a 3 BHK lifestyle apartment at Atita, a luxury project by LML Homes at Perungudi. The Apartment is worth every penny and the construction quality is good. The ethical work culture of LML Homes and the timely delivery by the LML Team headed by Mr, Mahesh Kothari has impressed us immensely. I will surely recommend prospective home buyers to buy homes from LML Homes",
            image: "/assets/images/testimonial-4.png",
            alt: "Testimonial from Mohandass about LML Homes' excellent service",
          },
          {
            name: "Mehar Nisha",
            location: "Tambaram",
            property: "LML Prakriti",
            description:
              "We would like to thank and appreciate the entire LML Homes team for the wonderful work and architecture of the LML Prakriti premises. We would like to thank Ilairaja for all the corporation and for working on our feedback effectively. We would also like to thank Rajesh and Dimple for their documentation support. Additionally, we would like to thank Vignesh and MD Mr. Mahesh Kothari for the explanation of the project very clear. Thanks to the entire LML Team. You guys are the best",
            image: "/assets/images/testimonial-2.png",
        alt: "Mehar Nisha's review highlighting LML Homes' top-notch quality",
          },
          {
            name: "Mrs. Vanitha",
            location: "Raga",
            property: "LML Raga",
            description:
              "I am so glad to buy a house with LML Raga. I am an Architect who takes a lot of time in minute details before finalizing a house. But, the decision to buy this house with LML Raga was finalized just 2 days after seeing the property. I must appreciate the efficiency of your staff too. Thank you for making this transition seamless. I highly recommend LML Raga.",
            image: "/assets/images/testimonial-1.png",
        alt: "Feedback from Vanitha on her new home at LML Homes",
            
          },
          {
            name: "Mr. Suman,",
            location: "Tambaram",
            property: "LML Prakriti",
            description:
              "Thanks much to LML Homes for helping me out to get a nice flat with expected Vaastu. I got this information from 99acres and the sales team shared the details and clarified my doubts very patiently. Thanks again to LML Homes for getting me nice flat near to my office MEPZ.",
            image: "/assets/images/testimonial-3.png",
        alt: "Review by Suman praising LML Homes' exceptional customer support",            
          },
          {
            name: "Ms. K. Preethi",
            location: "Tambaram",
            property: "LML Prakriti",
            description:
              'I would like to express my thanks for good communication and coordination for the registration. The site is pretty awesome and good and I am happy to have a sweet home from "LML Homes". The location of property is the very plus point, have ease of access to all the routes in Chennai. I recommend "LML Homes" based on kind attitude, communication and coordination from LML Team.',
            image: "/assets/images/testimonial-5.png",
        alt: "Feedback from K. Preethi about the seamless home-buying experience with LML Homes",
          },
        ],
        specifications: [
          {
            name: "structure",
            image: "construction.jpg",
            specifications: [
              {
                pointOne: "RCC framed structure with foundation designed to withstand Seismic loads",
                pointTwo: "Walls with solid blocks/AAC blocks in cement mortar",
              },
            ],
          },
          {
            name: "Flooring",
            image: "floor.jpg",
            specifications: [
              {
                pointOne:
                  "Living, dining and bedroom area will be finished with good quality 2x2 Vitrified tiles of builder's choice with necessary skirting.",
                // pointTwo: "Walls with solid blocks/AAC blocks in cement mortar"
              },
            ],
          },
          {
            name: "Plumbing & Sanitary",
            image: "pluming.jpg",
            specifications: [
              {
                pointOne: "Ceramic (Anti-skid) flooring in toilet. Bathroom walls covered with ceramic tiles",
                pointTwo: "Closets and washbasins in white Parryware",
              },
            ],
          },
          {
            name: "Windows & Ventilators",
            image: "window.jpg",
            specifications: [
              {
                pointOne: "Windows and Ventilators made of UPVC/Aluminium",
                pointTwo: "Provided with a clear glass pane",
              },
            ],
          },
          {
            name: "Painting",
            image: "painting.jpg",
            specifications: [
              {
                pointOne: "Living, dining and bedroom walls with two coat putty",
                pointTwo: "All wooden frames, shutters with enamel paint",
              },
            ],
          },
          {
            name: "Electrical",
            image: "electricity.jpg",
            specifications: [
              {
                pointOne: "Concealed copper wiring suitable for three-phase supply, with ISI quality",
                pointTwo: "Switches and sockets with adequate point for all",
              },
            ],
          },
          {
            name: "Water",
            image: "water-supply.jpg",
            specifications: [
              {
                pointOne: "Well and RCC sump of adequate capacity",
                pointTwo: "Hydro pneumatic system for water supply",
              },
            ],
          },
          {
            name: "Doors",
            image: "door.jpg",
            specifications: [
              {
                pointOne: "Main door frame of hardwood and shutter with solid core door of 35 mm",
                pointTwo: "SS finished door fitting for main door with Godrej or equivalent lock and door.",
              },
            ],
          },
        ],
      },
    },
    {
      slug: "iconia",
      name: "ICONIA",
      image: "assets/images/lml-map-img.jpg",
      location: "Iconia",
      price: "66 Lakhs",
      floor: "2BHK",
      description: "Prakriti is a luxurious residential complex featuring four thoughtfully designed",
      locationInfo: {
        lat: 13.9249,
        lng: 81.1,
      },
      detail: {
        bannerImage: "iconia-banner.jpg",
        logo: "Iconia-logo.svg",
        heading: "LML ICONIA, KATTUPAKKAM",
        keyLocation: "Behind Aravind Eye Hospital",
        detailDescription:
          "The Iconia is LML Homes 'dedication to the living icons of the city. Unique, magnificent, and exclusive, the Iconia is truly one-of-its-kind. Comprising just 82 luxury condos spread across 3 floors in 6 towers, the Iconia is not just another lifestyle apartment. It is a premium lifestyle statement of passionately planned eco-friendly homes in a contemporary setting. Coming up in 1.08 acres of pristine land in the serene locality just behind Aravind Eye Hospital, your home at Iconia will keep your lives peacefully tucked away in the midst of all that is essential to you. Making the ideal space for you and your family to live in complete joy is what the Iconia is designed to be. Facilitating this are the features and amenities packed into the project ",
        offer: {
          price: "66 Lakhs",
          info: "2BHK",
          subContent: "Excluding Registration (Ready to Occupy)",
        },
        pdf: "assets/pdfs/iconia-brochure.pdf",
        enquire: {
          whatsApp: "+918898989898",
          phone: "+918898989898",
        },
        bannerHighlights: [
          {
            icon: "",
            label: "2 BHK",
          },
          {
            icon: "",
            label: "GATED COMMUNITY OF 82 UNITS",
          },
          {
            icon: "",
            label: "CMDA RERA Approved",
          },
          {
            icon: "credaui-white.png",
            label: "",
          },
        ],
        descriptionHighlights: [
          {
            icon: "proj-high-2.svg",
            label: "2 BHK",
          },
          {
            icon: "proj-high-5.svg",
            label: "Gated Community",
          },
          {
            icon: "proj-high-4.svg",
            label: "6 Blocks",
          },
          {
            icon: "proj-high-3.svg",
            label: "Area 942 - 957 sq.ft",
          },
          {
            icon: "proj-high-6.svg",
            label: "CMDA RERA Approved",
          },
          {
            icon: "proj-high-2.svg",
            label: "Stilt + 3 Floors",
          },
        ],
        walkthrough: {
          video: "https://www.youtube.com/embed/3qZ_x2AVylU",
          description:
            "At LML Iconia, every day offers a new opportunity to immerse yourself in an extravagant lifestyle.",
          question: "WHY LML ICONIA?",
          answer:
            "An ideal home to unwind from the stress of everyday life, ICONIA is the ultimate destination to refresh, rejuvenate and relieve. Furthermore, this striking project is equipped with round-the-clock security & aided by hi-tech modern security systems. LML Iconia is iconic living by all standards. Standing out for its choice of location, architecture, and amenities, everything about LML Iconia makes it worth owning. ",
        },
        amenities: {
          description:
            "At Iconia, we welcome you to a world of state-of-the-art amenities and facilities that help you savour a premium lifestyle.",
          amenityList: [
            {
              image: "assets/prakriti/gym.png",
              label: "GYM",
            },
            {
              image: "assets/prakriti/cctv.png",
              label: "CCTV Surviellance",
            },
            {
              image: "assets/prakriti/Department-Store.png",
              label: "Departmental Store",
            },
            {
              image: "assets/prakriti/Kids-Play-Area.png",
              label: "Kids Play Area",
            },
            {
              image: "assets/prakriti/lift.png",
              label: "Elevator",
            },
            {
              image: "assets/prakriti/Mini-Hall.png",
              label: "Multi-Purpose Hall",
            },
            // {
            //   image: "assets/prakriti/Mini-Theatre.png",
            //   label: "Mini Theatre",
            // },
            {
              image: "assets/prakriti/Parking.png",
              label: "Parking",
            },
            {
              image: "assets/prakriti/security.png",
              label: "24x7 Security",
            },
            {
              image: "assets/prakriti/Sewage-Treatment-Plant.png",
              label: "Sewage Treatment Plant",
            },
            {
              image: "assets/prakriti/Temple.png",
              label: "Temple",
            },
            {
              image: "assets/prakriti/Walking-Path-way.png",
              label: "Walking-Path-way",
            },
          ],
        },
        locationInfo: {
          heading: "Location - 'kattupakkam' ",
          description:
            "Nestled in the vibrant city of Chennai, the gated community of Kattupakkam offers a serene and modern way of life, away from the hustle and bustle of the city. In this blog, we’ll take you on a virtual tour of this peaceful enclave, showcasing its lush landscapes, top-notch amenities, and the sense of community that makes it a sought-after destination for residents seeking both comfort and convenience.",
          CenterLocation: {
            lat: 13.052866210076996,
            lng: 80.12838114024606,
          },
          centerLocationMarker: "Iconia-logo.svg",
          centerLocationLink: "https://goo.gl/maps/6V8E3TBuEau7DAff6",
          locations: [
            {
              name: "Commute",
              icon: "commute.svg",
              locationList: [
                {
                  name: "ACS Medical College Bus stop - 500 mtrs",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Saveetha College Bus Stop - 800 mtrs",
                  location: {
                    lat: 13.054041626334048,
                    lng: 80.1237994284784,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Koyembedu Metro station- 9.4 Km",
                  location: {
                    lat: 13.07974899694047,
                    lng: 80.19577455935676,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 13.052541042946762,
                    lng: 80.1282115299084,
                    markerIcon: "Iconia-logo.svg",
                  },
                },
              ],
            },
            {
              name: "Education",
              icon: "education.svg",
              locationList: [
                {
                  name: "Saveetha College - 800 m",
                  location: {
                    lat: 13.061940343375163,
                    lng: 80.15590955104437,
                    markerIcon: "education-fill.svg",
                  },
                },

                {
                  name: "A.C.S Medical College - 1 km",
                  location: {
                    lat: 13.061940343375163,
                    lng: 80.15590955104437,
                    markerIcon: "education-fill.svg",
                  },
                },

                {
                  name: "Sana Model School - 1.2 km",
                  location: {
                    lat: 13.049367383317387,
                    lng: 80.12256144576085,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Pupil Pubilc School - 1.7 km",
                  location: {
                    lat: 13.060054510966326,
                    lng: 80.12297978956825,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 13.052541042946762,
                    lng: 80.1282115299084,
                    markerIcon: "Iconia-logo.svg",
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "DRR Eye Hospital-2 km",
                  location: {
                    lat: 13.04935821902995,
                    lng: 80.11465072984365,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Aravind Eye Hospital-600 mtrs",
                  location: {
                    lat: 13.05771813778298,
                    lng: 80.12323785646828,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Sri Ramachandra Hospital - 3.7km",
                  location: {
                    lat: 13.046558124410359,
                    lng: 80.14504128372614,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 13.052541042946762,
                    lng: 80.1282115299084,
                    markerIcon: "Iconia-logo.svg",
                  },
                },
              ],
            },
            {
              name: "commercial Hub",
              icon: "commercial-hub.svg",
              locationList: [
                {
                  name: "SB Cinemas, Kumananchavadi - 4.2 km",
                  location: {
                    lat: 13.058450725258574,
                    lng: 80.11816018499428,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "Sri Vigneswara Theatre, Poonamallee - 4.3 km",
                  location: {
                    lat: 13.064943633474133,
                    lng: 80.09278706486852,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "Malliga Theatre, Kunrathur - 4.6 km",
                  location: {
                    lat: 13.040288810525338,
                    lng: 80.11159440032003,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 13.052541042946762,
                    lng: 80.1282115299084,
                    markerIcon: "Iconia-logo.svg",
                  },
                },
              ],
            },
          ],
          address: "No.9, Big Street / Pillayar Koil Street,Kattupakkam, Behind Aravind Eye Hospital",
        },
        layout: {
          heading: "SITE PLAN",
          // description:
          //   "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "Key Plan",
              image: "/assets/arshiya/arshiya-KeyPlan.jpg",
            },
            {
              name: "Block A",
              image: "/assets/iconia/Block-A.jpg",
            },
            {
              name: "Block B",
              image: "/assets/iconia/Block-B.jpg",
            },
            {
              name: "Block C",
              image: "/assets/iconia/Block-C.jpg",
            },
            {
              name: "Block D",
              image: "/assets/iconia/Block-D.jpg",
            },
            {
              name: "Block E",
              image: "/assets/iconia/Block-E.jpg",
            },
            {
              name: "Block F",
              image: "/assets/iconia/Block-F.jpg",
            },
          ],
        },
        unitPlan: {
          heading: "Unit Plans",
          // description:
          //   "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
              ],
            },
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
              ],
            },
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
              ],
            },
          ],
        },
        subBannerImage: "/iconia/iconia-sub-banner.png",
        gallery: {
          heading: "Glimpses of life at prakarthi",
          // subHeading:
          // "With beautiful and thoughtfully-designed homes at kattupakkam, you can experience the sunny side of life.",
          list: [
            {
              name: "Actual Image",
              images: [
                {
                  image: "/iconia/interior/interior-1.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-2.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-3.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-4.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-5.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-6.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-7.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-8.jpg",
                  label: "Interior View",
                },

                {
                  image: "/iconia/interior/interior-9.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-10.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-11.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-12.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-13.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-14.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-15.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-16.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-17.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-18.jpg",
                  label: "Interior View",
                },
                {
                  image: "/iconia/interior/interior-19.jpg",
                  label: "Interior View",
                },
              ],
            },
            {
              name: "Project Image",
              images: [
                {
                  image: "/iconia/exterior/exterior-1.png",
                  label: "Exterior View",
                },
                {
                  image: "/iconia/exterior/exterior-2.png",
                  label: "Exterior View",
                },
                {
                  image: "/iconia/exterior/exterior-3.png",
                  label: "Exterior View",
                },
                {
                  image: "/iconia/exterior/exterior-4.png",
                  label: "Exterior View",
                },
                {
                  image: "/iconia/exterior/exterior-5.png",
                  label: "Exterior View",
                },
                {
                  image: "/iconia/exterior/exterior-7.jpeg",
                  label: "Exterior View",
                },
                {
                  image: "/iconia/exterior/exterior-10.jpeg",
                  label: "Exterior View",
                },
                {
                  image: "/iconia/exterior/exterior-12.jpeg",
                  label: "Exterior View",
                },
              ],
            },
          ],
        },
        testimonial: [
          {
            name: "Mr. Mohandass",
            location: "Perungudi",
            property: "LML Atita",
            description:
              "We own a 3 BHK lifestyle apartment at Atita, a luxury project by LML Homes at Perungudi. The Apartment is worth every penny and the construction quality is good. The ethical work culture of LML Homes and the timely delivery by the LML Team headed by Mr, Mahesh Kothari has impressed us immensely. I will surely recommend prospective home buyers to buy homes from LML Homes",
            image: "/assets/images/testimonial-4.png",
            alt: "Testimonial from Mohandass about LML Homes' excellent service",
          },
          {
            name: "Mehar Nisha",
            location: "Tambaram",
            property: "LML Prakriti",
            description:
              "We would like to thank and appreciate the entire LML Homes team for the wonderful work and architecture of the LML Prakriti premises. We would like to thank Ilairaja for all the corporation and for working on our feedback effectively. We would also like to thank Rajesh and Dimple for their documentation support. Additionally, we would like to thank Vignesh and MD Mr. Mahesh Kothari for the explanation of the project very clear. Thanks to the entire LML Team. You guys are the best",
            image: "/assets/images/testimonial-2.png",
            alt: "Mehar Nisha's review highlighting LML Homes' top-notch quality",
          },
          {
            name: "Mrs. Vanitha",
            location: "Raga",
            property: "LML Raga",
            description:
              "I am so glad to buy a house with LML Raga. I am an Architect who takes a lot of time in minute details before finalizing a house. But, the decision to buy this house with LML Raga was finalized just 2 days after seeing the property. I must appreciate the efficiency of your staff too. Thank you for making this transition seamless. I highly recommend LML Raga.",
            image: "/assets/images/testimonial-1.png",
            alt: "Feedback from Vanitha on her new home at LML Homes",
          },
          {
            name: "Mr. Suman,",
            location: "Tambaram",
            property: "LML Prakriti",
            description:
              "Thanks much to LML Homes for helping me out to get a nice flat with expected Vaastu. I got this information from 99acres and the sales team shared the details and clarified my doubts very patiently. Thanks again to LML Homes for getting me nice flat near to my office MEPZ.",
            image: "/assets/images/testimonial-3.png",
            alt: "Review by Suman praising LML Homes' exceptional customer support",
          },
          {
            name: "Ms. K. Preethi",
            location: "Tambaram",
            property: "LML Prakriti",
            description:
              'I would like to express my thanks for good communication and coordination for the registration. The site is pretty awesome and good and I am happy to have a sweet home from "LML Homes". The location of property is the very plus point, have ease of access to all the routes in Chennai. I recommend "LML Homes" based on kind attitude, communication and coordination from LML Team.',
            image: "/assets/images/testimonial-5.png",
            alt: "Feedback from K. Preethi about the seamless home-buying experience with LML Homes",
          }
        ],
        specifications: [
          {
            name: "structure",
            image: "construction.jpg",
            specifications: [
              {
                pointOne: "RCC framed structure with foundation designed to withstand Seismic loads",
                pointTwo: "Walls with solid blocks/AAC blocks in cement mortar",
              },
            ],
          },
          {
            name: "Flooring",
            image: "floor.jpg",
            specifications: [
              {
                pointOne:
                  "Living, dining and bedroom area will be finished with good quality 2x2 Vitrified tiles of builder's choice with necessary skirting.",
                // pointTwo: "Walls with solid blocks/AAC blocks in cement mortar"
              },
            ],
          },
          {
            name: "Plumbing & Sanitary",
            image: "pluming.jpg",
            specifications: [
              {
                pointOne: "Ceramic (Anti-skid) flooring in toilet. Bathroom walls covered with ceramic tiles",
                pointTwo: "Closets and washbasins in white Parryware",
              },
            ],
          },
          {
            name: "Windows & Ventilators",
            image: "window.jpg",
            specifications: [
              {
                pointOne: "Windows and Ventilators made of UPVC/Aluminium",
                pointTwo: "Provided with a clear glass pane",
              },
            ],
          },
          {
            name: "Painting",
            image: "painting.jpg",
            specifications: [
              {
                pointOne: "Living, dining and bedroom walls with two coat putty",
                pointTwo: "All wooden frames, shutters with enamel paint",
              },
            ],
          },
          {
            name: "Electrical",
            image: "electricity.jpg",
            specifications: [
              {
                pointOne: "Concealed copper wiring suitable for three-phase supply, with ISI quality",
                pointTwo: "Switches and sockets with adequate point for all",
              },
            ],
          },
          {
            name: "Water",
            image: "water-supply.jpg",
            specifications: [
              {
                pointOne: "Well and RCC sump of adequate capacity",
                pointTwo: "Hydro pneumatic system for water supply",
              },
            ],
          },
          {
            name: "Doors",
            image: "door.jpg",
            specifications: [
              {
                pointOne: "Main door frame of hardwood and shutter with solid core door of 35 mm",
                pointTwo: "SS finished door fitting for main door with Godrej or equivalent lock and door.",
              },
            ],
          },
        ],
        address: [
          {
            address: "No.9, Big Street / Pillayar Koil Street,Kattupakkam,Behind Aravind Eye Hospital",
          },
        ],
      },
    },
    {
      slug: "arshiya",
      name: "ARSHIYA",
      image: "assets/images/lml-map-img.jpg",
      location: "Kattupakkam",
      floor: "2BHK | 3BHK",
      price: "69 Lakhs",
      locationInfo: {
        lat: 12.9249,
        lng: 80.1,
      },
      detail: {
        bannerImage: "arshiya-banner.jpg",
        logo: "Arshiya-logo.svg",
        heading: "LML ARSHIYA, KATTUPAKKAM",
        keyLocation: "",
        detailDescription:
          "LML Homes proudly presents Arshiya, a prestigious residential community at Kattupakkam in Chennai. Covering a generous expanse of 84.5 cents, LML Arshiya communicates a spirit of life and zest by accommodating 74 comfort-packed apartments.",
        offer: {
          price: "69 Lakhs",
          info: "2&3 BHK",
          subContent: "Excluding Reg & Gst (Ready to Fitout)",
        },
        pdf: "assets/pdfs/arshiya-brochure.pdf",
        enquire: {
          whatsApp: "+918898989898",
          phone: "+918898989898",
        },
        bannerHighlights: [
          {
            icon: "",
            label: "2&3 BHK",
          },
          {
            icon: "",
            label: "GATED COMMUNITY OF 74 UNITS",
          },
          {
            icon: "",
            label: "CMDA RERA Approved",
          },
          {
            icon: "credaui-white.png",
            label: "",
          },
        ],
        descriptionHighlights: [
          {
            icon: "proj-high-2.svg",
            label: "2&3 BHK",
          },
          {
            icon: "proj-high-5.svg",
            label: "Gated Community",
          },
          {
            icon: "proj-high-4.svg",
            label: "3 Blocks",
          },
          {
            icon: "proj-high-3.svg",
            label: "992 - 1399 sq.ft",
          },
          {
            icon: "proj-high-6.svg",
            label: "CMDA RERA Approved",
          },
          {
            icon: "proj-high-2.svg",
            label: "Stilt + 5 Floors",
          },
        ],
        walkthrough: {
          video: "https://www.youtube.com/embed/NBandYORz9w",
          description:
            "At LML Arshiya, every day offers a new opportunity to immerse yourself in an extravagant lifestyle.",
          question: "WHY LML ARSHIYA?",
          answer:
            "Arshiya is an exquisite collection of elegant and spacious apartments designed to provide the perfect living spaces for you to live the life you always wanted. Arshiya is truly an appreciating asset with fast accessibility to the employment zones of Porur, Manapakkam, Avadi, Ambattur, and the city. Set amidst the touch points of every day, Arshiya is advantageously close to educational institutions, healthcare centers, hospitality establishments, and entertainment hubs.",
        },
        amenities: {
          description:
            "At Arshiya, we welcome you to a world of state-of-the-art amenities and facilities that help you savour a premium lifestyle.",
          amenityList: [
            {
              image: "assets/prakriti/gym.png",
              label: "GYM",
            },
            {
              image: "assets/prakriti/cctv.png",
              label: "CCTV Surviellance",
            },
            {
              image: "assets/prakriti/Department-Store.png",
              label: "Departmental Store",
            },
            {
              image: "assets/prakriti/Kids-Play-Area.png",
              label: "Kids Play Area",
            },
            {
              image: "assets/prakriti/lift.png",
              label: "Elevator",
            },
            {
              image: "assets/prakriti/Mini-Hall.png",
              label: "Multi-Purpose Hall",
            },
            // {
            //   image: "assets/prakriti/Mini-Theatre.png",
            //   label: "Mini Theatre",
            // },
            {
              image: "assets/prakriti/Parking.png",
              label: "Parking",
            },
            {
              image: "assets/prakriti/security.png",
              label: "24x7 Security",
            },
            {
              image: "assets/prakriti/Sewage-Treatment-Plant.png",
              label: "Sewage Treatment Plant",
            },
            {
              image: "assets/prakriti/Temple.png",
              label: "Temple",
            },
            {
              image: "assets/prakriti/Walking-Path-way.png",
              label: "Walking-Path-way",
            },
          ],
        },
        locationInfo: {
          heading: "Location - 'KATTUPAKKAM' ",
          description:
            "This gated community apartments is the perfect place to find a home that's conveniently close to all the key infrastructures you need. With just a 5-minute drive, you can reach college, office, hospitals, malls and more.",
          CenterLocation: {
            lat: 13.050571663166105,
            lng: 80.12730457888271,
          },
          centerLocationMarker: "Arshiya-logo.svg",
          centerLocationLink: "https://goo.gl/maps/NUERaHYMp65hXz587",
          locations: [
            {
              name: "Commute",
              icon: "commute.svg",
              locationList: [
                {
                  name: "Porur - 3.5 km",
                  location: {
                    lat: 13.038111906199216,
                    lng: 80.15685989716447,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Iyyappanthangal Junction - 2.5 km",
                  location: {
                    lat: 13.047435601226757,
                    lng: 80.14046585321519,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Mangadu Bus stop - 3.7 kms",
                  location: {
                    lat: 13.040776035973392,
                    lng: 80.10989828906754,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "Arshiya-logo.svg",
                  },
                },
              ],
            },
            {
              name: "Education",
              icon: "education.svg",
              locationList: [
                {
                  name: "St. John’s School - 650 mts",
                  location: {
                    lat: 13.053015266720706,
                    lng: 80.13194783182286,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Aadhitya International public school - 300 mts",
                  location: {
                    lat: 13.047980660826648,
                    lng: 80.12680478536993,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Little Millennium School - 1.1 Km",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "Arshiya-logo.svg",
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Aravind Eye Hospital - 750 m",
                  location: {
                    lat: 13.053669957560038,
                    lng: 80.1238745825864,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Dr Mehta’s Hospital - 2.7 Km",
                  location: {
                    lat: 13.06415556083337,
                    lng: 80.13609279153108,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "DR Multispeciality Hospital - 2.5 Km",
                  location: {
                    lat: 13.039937372904518,
                    lng: 80.13565865254215,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "Arshiya-logo.svg",
                  },
                },
              ],
            },
            {
              name: "Retail",
              icon: "retail.svg",
              locationList: [
                {
                  name: "D-Mart - 1.8 Km",
                  location: {
                    lat: 13.0486999597234,
                    lng: 80.11862683314995,
                    markerIcon: "retail-fill.svg",
                  },
                },
                {
                  name: "Aishwaryam Mini Super Market - 2.4 Km",
                  location: {
                    lat: 13.04418589870694,
                    lng: 80.11347320137753,
                    markerIcon: "retail-fill.svg",
                  },
                },
                {
                  name: "Grace Super Market - 1.9 Km",
                  location: {
                    lat: 13.044444234252813,
                    lng: 80.13068165845044,
                    markerIcon: "retail-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "Arshiya-logo.svg",
                  },
                },
              ],
            },
            {
              name: "Commercial Hubs",
              icon: "commercial-hub.svg",
              locationList: [
                {
                  name: "S.B Cinemas - 2.3 Km",
                  location: {
                    lat: 13.058450725258574,
                    lng: 80.11816018499428,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "Gokulam Cinemas - 6 Km",
                  location: {
                    lat: 13.064228011765714,
                    lng: 80.08649601188743,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "Inox National Virugambakkam - 10.8 Km",
                  location: {
                    lat: 13.063075257944421,
                    lng: 80.19444453593566,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "Arshiya-logo.svg",
                  },
                },
              ],
            },
          ],
          address: "Sridevi Nagar 3rd street, Goparasanallur, Kattupakkam, Chennai - 600 056",
        },
        layout: {
          heading: "SITE PLAN",
          // description:
          //   "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "Key Plan",
              image: "/assets/iconia/arshiya-keyPlan.jpg",
            },
            {
              name: "Block-1 First Floor",
              image: "/assets/arshiya/block-1-first.jpg",
            },
            {
              name: "Block-1 Typical Floor",
              image: "/assets/arshiya/block-1-typical.jpg",
            },
            {
              name: "Block-2 Typical Floor",
              image: "/assets/arshiya/block-2-typical.jpg",
            },
            {
              name: "Block-3 Typical Floor",
              image: "/assets/arshiya/block-3-typical.jpg",
            },
          ],
        },
        unitPlan: {
          heading: "Unit Plans",
          // description:
          //   "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1013 SQFT",
                  image: "/assets/prakriti/B_403.jpg",
                },
                {
                  name: "1014 SQFT",
                  image: "/assets/prakriti/B_405.jpg",
                },
                {
                  name: "1004 SQFT",
                  image: "/assets/prakriti/B_406.jpg",
                },
                {
                  name: "1019 SQFT",
                  image: "/assets/prakriti/B_410.jpg",
                },
                {
                  name: "1024 SQFT",
                  image: "/assets/prakriti/C_304.jpg",
                },
                {
                  name: "1023 SQFT",
                  image: "/assets/prakriti/C_309.jpg",
                },
                {
                  name: "1044 SQFT",
                  image: "/assets/prakriti/C_402.jpg",
                },
                {
                  name: "1024 SQFT",
                  image: "/assets/prakriti/C_404.jpg",
                },
              ],
            },
            {
              name: "3BHK",
              sizeList: [
                {
                  name: "1395 SQFT",
                  image: "/assets/prakriti/B_413.jpg",
                },
                {
                  name: "1362 SQFT",
                  image: "/assets/prakriti/B_409.jpg",
                },
                {
                  name: "1310 SQFT",
                  image: "/assets/prakriti/C_411.jpg",
                },
                {
                  name: "1362 SQFT",
                  image: "/assets/prakriti/B_409.jpg",
                },
                {
                  name: "1027 SQFT",
                  image: "/assets/prakriti/C_405.jpg",
                },
                {
                  name: "1272 SQFT",
                  image: "/assets/prakriti/C_406.jpg",
                },
                {
                  name: "1008 SQFT",
                  image: "/assets/prakriti/C_408.jpg",
                },
                {
                  name: "1023 SQFT",
                  image: "/assets/prakriti/C_409.jpg",
                },
                {
                  name: "598 SQFT",
                  image: "/assets/prakriti/E_408a.jpg",
                },
              ],
            },
          ],
        },
        subBannerImage: "/arshiya/arshiya-sub-banner.png",
        gallery: {
          heading: "Glimpses of life at prakarthi",
          // subHeading:
          // "With beautiful and thoughtfully-designed homes at kattupakkam, you can experience the sunny side of life.",
          list: [
            {
              name: "Actual Image",
              images: [
                {
                  image: "/arshiya/gallery/gallery-6.jpg",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-8.jpg",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-9.jpg",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-10.jpg",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-12.jpg",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-13.jpg",
                  label: "Project Image",
                },

                {
                  image: "/arshiya/gallery/gallery-15.jpg",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-16.jpg",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-17.jpg",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-18.jpg",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-19.jpg",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-20.jpg",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-21.jpg",
                  label: "Project Image",
                },
              ],
            },
            {
              name: "Project Image",
              images: [
                {
                  image: "/arshiya/gallery/gallery-1.png",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-2.png",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-3.png",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-4.png",
                  label: "Project Image",
                },
                {
                  image: "/arshiya/gallery/gallery-5.png",
                  label: "Project Image",
                },             
              ],
            },
          ],
        },

        testimonial: [
          {
            name: "Mr. Mohandass",
            location: "Perungudi",
            property: "LML Atita",
            description:
              "We own a 3 BHK lifestyle apartment at Atita, a luxury project by LML Homes at Perungudi. The Apartment is worth every penny and the construction quality is good. The ethical work culture of LML Homes and the timely delivery by the LML Team headed by Mr, Mahesh Kothari has impressed us immensely. I will surely recommend prospective home buyers to buy homes from LML Homes",
            image: "/assets/images/testimonial-4.png",
            alt: "Testimonial from Mohandass about LML Homes' excellent service",
          },
          {
            name: "Mehar Nisha",
            location: "Tambaram",
            property: "LML Prakriti",
            description:
              "We would like to thank and appreciate the entire LML Homes team for the wonderful work and architecture of the LML Prakriti premises. We would like to thank Ilairaja for all the corporation and for working on our feedback effectively. We would also like to thank Rajesh and Dimple for their documentation support. Additionally, we would like to thank Vignesh and MD Mr. Mahesh Kothari for the explanation of the project very clear. Thanks to the entire LML Team. You guys are the best",
            image: "/assets/images/testimonial-2.png",
             alt: "Mehar Nisha's review highlighting LML Homes' top-notch quality",
          },
          {
            name: "Mrs. Vanitha",
            location: "Raga",
            property: "LML Raga",
            description:
              "I am so glad to buy a house with LML Raga. I am an Architect who takes a lot of time in minute details before finalizing a house. But, the decision to buy this house with LML Raga was finalized just 2 days after seeing the property. I must appreciate the efficiency of your staff too. Thank you for making this transition seamless. I highly recommend LML Raga.",
            image: "/assets/images/testimonial-1.png",
            alt: "Feedback from Vanitha on her new home at LML Homes",
          },
          {
            name: "Mr. Suman,",
            location: "Tambaram",
            property: "LML Prakriti",
            description:
              "Thanks much to LML Homes for helping me out to get a nice flat with expected Vaastu. I got this information from 99acres and the sales team shared the details and clarified my doubts very patiently. Thanks again to LML Homes for getting me nice flat near to my office MEPZ.",
            image: "/assets/images/testimonial-3.png",
            alt: "Review by Suman praising LML Homes' exceptional customer support",
          },
          {
            name: "Ms. K. Preethi",
            location: "Tambaram",
            property: "LML Prakriti",
            description:
              'I would like to express my thanks for good communication and coordination for the registration. The site is pretty awesome and good and I am happy to have a sweet home from "LML Homes". The location of property is the very plus point, have ease of access to all the routes in Chennai. I recommend "LML Homes" based on kind attitude, communication and coordination from LML Team.',
            image: "/assets/images/testimonial-5.png",
            alt: "Feedback from K. Preethi about the seamless home-buying experience with LML Homes",
          },
        ],
        specifications: [
          {
            name: "structure",
            image: "construction.jpg",
            specifications: [
              {
                pointOne: "RCC framed structure with foundation designed to withstand Seismic loads",
                pointTwo: "Walls with solid blocks/AAC blocks in cement mortar",
              },
            ],
          },
          {
            name: "Flooring",
            image: "floor.jpg",
            specifications: [
              {
                pointOne:
                  "Living, dining and bedroom area will be finished with good quality 2x2 Vitrified tiles of builder's choice with necessary skirting.",
                // pointTwo: "Walls with solid blocks/AAC blocks in cement mortar"
              },
            ],
          },
          {
            name: "Plumbing & Sanitary",
            image: "pluming.jpg",
            specifications: [
              {
                pointOne: "Ceramic (Anti-skid) flooring in toilet. Bathroom walls covered with ceramic tiles",
                pointTwo: "Closets and washbasins in white Parryware",
              },
            ],
          },
          {
            name: "Windows & Ventilators",
            image: "window.jpg",
            specifications: [
              {
                pointOne: "Windows and Ventilators made of UPVC/Aluminium",
                pointTwo: "Provided with a clear glass pane",
              },
            ],
          },
          {
            name: "Painting",
            image: "painting.jpg",
            specifications: [
              {
                pointOne: "Living, dining and bedroom walls with two coat putty",
                pointTwo: "All wooden frames, shutters with enamel paint",
              },
            ],
          },
          {
            name: "Electrical",
            image: "electricity.jpg",
            specifications: [
              {
                pointOne: "Concealed copper wiring suitable for three-phase supply, with ISI quality",
                pointTwo: "Switches and sockets with adequate point for all",
              },
            ],
          },
          {
            name: "Water",
            image: "water-supply.jpg",
            specifications: [
              {
                pointOne: "Well and RCC sump of adequate capacity",
                pointTwo: "Hydro pneumatic system for water supply",
              },
            ],
          },
          {
            name: "Doors",
            image: "door.jpg",
            specifications: [
              {
                pointOne: "Main door frame of hardwood and shutter with solid core door of 35 mm",
                pointTwo: "SS finished door fitting for main door with Godrej or equivalent lock and door.",
              },
            ],
          },
        ],
      },
    },
    {
      slug: "league-one",
      name: "THE LEAGUE ONE",
      image: "assets/images/lml-map-img.jpg",
      location: "Kattupakkam",
      floor: "2&3 BHK",
      price: "66 Lakhs",
      locationInfo: {
        lat: 12.9249,
        lng: 80.1,
      },
      detail: {
        bannerImage: "league-one-banner.jpg",
        // logo: "league one logo.png",
        heading: "LML The League One, Kaatupakkam",
        keyLocation: "",
        detailDescription:
          "Experience extraordinary living in Kattupakkam. Luxurious 2 & 3 BHK residences offer unmatched privacy in just 125 units. Thoughtfully designed for comfort and space, each home is a haven of tranquillity. Live exceptionally at LML The League One.",
        offer: {
          price: "66 Lakhs",
          info: "2&3 BHK",
          subContent: "",
        },
        pdf: "assets/pdfs/theleagueone.pdf",
        enquire: {
          whatsApp: "+918898989898",
          phone: "+918898989898",
        },
        bannerHighlights: [
          {
            icon: "",
            label: "2&3 BHK",
          },
          {
            icon: "",
            label: "GATED COMMUNITY WITH 125 UNITS",
          },
          {
            icon: "",
            label: "RERA TN/2/BUILDING/0405/2024",
          },
          {
            icon: "credaui-white.png",
            label: "",
          },
        ],
        descriptionHighlights: [
          {
            icon: "proj-high-2.svg",
            label: "2 & 3 BHK",
          },
          {
            icon: "proj-high-5.svg",
            label: "Gated Community",
          },
          {
            icon: "proj-high-4.svg",
            label: "4 Blocks",
          },
          {
            icon: "proj-high-3.svg",
            label: "993 - 1426 sq.ft",
          },
          {
            icon: "proj-high-6.svg",
            label: "RERA & CMDA Approved",
          },
          {
            icon: "proj-high-2.svg",
            label: "1.22 Acres",
          },
          {
            icon: "clubhouse.png",
            label: "9000+ sq.ft Club House",
          },
        ],
        walkthrough: {
          video: "https://www.youtube.com/embed/s5L1p_4k6s0?si=wc3IPZKGf5ZctQrM",
          // video: "",
          description:
            "At LML The League One, every day offers a new opportunity to immerse yourself in an extravagant lifestyle.",
          question: "WHY LML The League One?",
          answer:
            "Arshiya is an exquisite collection of elegant and spacious apartments designed to provide the perfect living spaces for you to live the life you always wanted. Arshiya is truly an appreciating asset with fast accessibility to the employment zones of Porur, Manapakkam, Avadi, Ambattur, and the city. Set amidst the touch points of every day, Arshiya is advantageously close to educational institutions, healthcare centers, hospitality establishments, and entertainment hubs.",
        },
        amenities: {
          description:
            "At LML The League One, we welcome you to a world of state-of-the-art amenities and facilities that help you savour a premium lifestyle.",
          amenityList: [
            // {
            //   image: "assets/prakriti/gym.png",
            //   label: "GYM",
            // },
            {
              image: "assets/prakriti/cctv.png",
              label: "24/7 Surveillance Cabin",
            },
            {
              image: "assets/league-one/badminton_court.jpg",
              label: "Badminton Court",
            },
            {
              image: "assets/prakriti/Kids-Play-Area.png",
              label: "Kids Play Area",
            },
            {
              image: "assets/prakriti/lift.png",
              label: "Automated Passenger Lift",
            },
            {
              image: "assets/prakriti/Mini-Hall.png",
              label: "Multi-Purpose Hall",
            },
            {
              image: "assets/league-one/grand_entrance.jpg",
              label: "Grand Entrance Arch",
            },
            {
              image: "assets/prakriti/Parking.png",
              label: "EV Charging Point",
            },
            {
              image: "assets/prakriti/Sewage-Treatment-Plant.png",
              label: "Sewage Treatment Plant",
            },
            {
              image: "assets/prakriti/security.png",
              label: "Paved Driveway",
            },
            {
              image: "assets/league-one/club_house.jpg",
              label: "Clubhouse",
            },
            {
              image: "assets/prakriti/Walking-Path-way.png",
              label: "Wellness Zone",
            },
          ],
        },
        locationInfo: {
          heading: "Location - 'KATTUPAKKAM' ",
          description:
            "This gated community apartment is the perfect place to find a home that's conveniently close to all the key infrastructures you need. Now, reach educational institutes, hospitals, and malls that are just a 5-minute drive away.",
          CenterLocation: {
            // lat: 13.050571663166105,
            lat: 13.048234,
            // lng: 80.12730457888271,
            lng: 80.122341,
          },
          centerLocationMarker: "league one logo.png",
          centerLocationLink: "https://maps.app.goo.gl/UCd1DhVheHaaSoHN6",
          locations: [
            {
              name: "Connectivity",
              icon: "commute.svg",
              locationList: [
                {
                  name: "Proposed metro - 2 mins",
                  location: {
                    lat: 13.038111906199216,
                    lng: 80.15685989716447,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Porur - 5 mins",
                  location: {
                    lat: 13.047435601226757,
                    lng: 80.14046585321519,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Iyyappanthangal Junction – 3mins",
                  location: {
                    lat: 13.040776035973392,
                    lng: 80.10989828906754,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Vanagaram Junction – 5mins",
                  location: {
                    lat: 13.040776035973392,
                    lng: 80.10989828906754,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Airport – 20 mins",
                  location: {
                    lat: 13.040776035973392,
                    lng: 80.10989828906754,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Poonamallee bus Terminus – 5mins",
                  location: {
                    lat: 13.040776035973392,
                    lng: 80.10989828906754,
                    markerIcon: "commute-fill.svg",
                  },
                },
                {
                  name: "Thiruverkadu Junction – 5mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Guindy railway Station – 15mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Koyambedu – 10mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "15 mins to IT corridor, DLF, L&T, RMZ, CommerZone",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
              ],
            },
            {
              name: "Schools",
              icon: "education.svg",
              locationList: [
                {
                  name: "Velammal School - 5 mins",
                  location: {
                    lat: 13.053015266720706,
                    lng: 80.13194783182286,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Sindhi School - 5 mins",
                  location: {
                    lat: 13.047980660826648,
                    lng: 80.12680478536993,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Maharishi Vidya Mandir, Mangadu - 10 mins",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Little Millenium School - 2 mins",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Rishi International School - 2 mins",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Adithya International School - 2 mins",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "St Johns School - 5 mins",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Christ Matric High Sec School - 5 mins",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Sana Model School - 0 mins",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Narayana E-Techno - 5 mins",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Adithya Vidyashram - 10 mins",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Sudharsan Vidyashram - 10 mins",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Chaitanya School - 5 mins",
                  location: {
                    lat: 13.051099754391325,
                    lng: 80.12389255597654,
                    markerIcon: "education-fill.svg",
                  },
                },
                {
                  name: "Pupil School - 10 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Vedanta School - 10 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "The Schram Academy - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
              ],
            },
            {
              name: "Colleges",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "ACS Medical College - 10 mins",
                  location: {
                    lat: 13.053669957560038,
                    lng: 80.1238745825864,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Saveetha Dental College - 10 mins",
                  location: {
                    lat: 13.06415556083337,
                    lng: 80.13609279153108,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Sindhi College - 5 mins",
                  location: {
                    lat: 13.039937372904518,
                    lng: 80.13565865254215,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Apollo Nursing College - 10 mins",
                  location: {
                    lat: 13.039937372904518,
                    lng: 80.13565865254215,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Meenakshi Dental College - 10 mins",
                  location: {
                    lat: 13.039937372904518,
                    lng: 80.13565865254215,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "Saveetha B School - 5 mins",
                  location: {
                    lat: 13.039937372904518,
                    lng: 80.13565865254215,
                    markerIcon: "healthcare-fill.svg",
                  },
                },
                {
                  name: "SA Engineering College - 10 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "MGR University - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "DMI College of Engineering - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Kings Engineering College - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "St.Peters Engineering College - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Rajalakshmi B School - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Panimalar Engineering College - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
              ],
            },
            {
              name: "Hospitals",
              icon: "retail.svg",
              locationList: [
                {
                  name: "Aravind Eye Hospital - 5 mins",
                  location: {
                    lat: 13.0486999597234,
                    lng: 80.11862683314995,
                    markerIcon: "retail-fill.svg",
                  },
                },
                {
                  name: "Ramachandra Hospital - 10 mins",
                  location: {
                    lat: 13.04418589870694,
                    lng: 80.11347320137753,
                    markerIcon: "retail-fill.svg",
                  },
                },
                {
                  name: "Dr. Mehta Hospital - 5 mins",
                  location: {
                    lat: 13.044444234252813,
                    lng: 80.13068165845044,
                    markerIcon: "retail-fill.svg",
                  },
                },
                {
                  name: "Saveetha Dental Hospital - 5 mins",
                  location: {
                    lat: 13.044444234252813,
                    lng: 80.13068165845044,
                    markerIcon: "retail-fill.svg",
                  },
                },
                {
                  name: "Apollo Hospital - 10 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Be Well Hospital - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
              ],
            },
            {
              name: "Convenience Hotspots",
              icon: "commercial-hub.svg",
              locationList: [
                {
                  name: "SB Cinemas - 5 mins",
                  location: {
                    lat: 13.058450725258574,
                    lng: 80.11816018499428,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "D Mart - 5 mins",
                  location: {
                    lat: 13.064228011765714,
                    lng: 80.08649601188743,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "Reliance Mall - 10 mins",
                  location: {
                    lat: 13.063075257944421,
                    lng: 80.19444453593566,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "Reliance Fresh - 5 mins",
                  location: {
                    lat: 13.063075257944421,
                    lng: 80.19444453593566,
                    markerIcon: "commercial-hub-fill.svg",
                  },
                },
                {
                  name: "Pazhamudhir Cholai - 5 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Saravana Stores - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Nilgiris - 5 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "GRACE Supermarket - 5 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Vigneshwara Theatre - 10 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Inox Theatre - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "GK Cinemas - 10 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Saravana Stores - 5.5kms",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "SAGS Maduravoyal - 0 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "EVP Cinemas - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
                {
                  name: "Queensland - 15 mins",
                  location: {
                    lat: 13.050571663166105,
                    lng: 80.12730457888271,
                    markerIcon: "league one logo.png",
                  },
                },
              ],
            },
          ],
          address: "Mount Poonamallee Rd, Goparasa Nallur, Kattupakkam, Chennai, Tamil Nadu 600 056",
        },
        layout: {
          heading: "SITE PLAN",
          // description:
          //   "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "Block A",
              image: "/assets/league-one/Block_A.png",
            },
            {
              name: "Block B",
              image: "/assets/league-one/Block_B.png",
            },
            {
              name: "Block C",
              image: "/assets/league-one/Block_C.png",
            },
            {
              name: "Block D",
              image: "/assets/league-one/Block_D.png",
            },
            {
              name: "Floor Plans",
              image: "/assets/league-one/Floor Plans.png",
            },
          ],
        },
        unitPlan: {
          heading: "Unit Plans",
          // description:
          //   "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1013 SQFT",
                  image: "/assets/prakriti/B_403.jpg",
                },
                {
                  name: "1014 SQFT",
                  image: "/assets/prakriti/B_405.jpg",
                },
                {
                  name: "1004 SQFT",
                  image: "/assets/prakriti/B_406.jpg",
                },
                {
                  name: "1019 SQFT",
                  image: "/assets/prakriti/B_410.jpg",
                },
                {
                  name: "1024 SQFT",
                  image: "/assets/prakriti/C_304.jpg",
                },
                {
                  name: "1023 SQFT",
                  image: "/assets/prakriti/C_309.jpg",
                },
                {
                  name: "1044 SQFT",
                  image: "/assets/prakriti/C_402.jpg",
                },
                {
                  name: "1024 SQFT",
                  image: "/assets/prakriti/C_404.jpg",
                },
              ],
            },
            {
              name: "3BHK",
              sizeList: [
                {
                  name: "1395 SQFT",
                  image: "/assets/prakriti/B_413.jpg",
                },
                {
                  name: "1362 SQFT",
                  image: "/assets/prakriti/B_409.jpg",
                },
                {
                  name: "1310 SQFT",
                  image: "/assets/prakriti/C_411.jpg",
                },
                {
                  name: "1362 SQFT",
                  image: "/assets/prakriti/B_409.jpg",
                },
                {
                  name: "1027 SQFT",
                  image: "/assets/prakriti/C_405.jpg",
                },
                {
                  name: "1272 SQFT",
                  image: "/assets/prakriti/C_406.jpg",
                },
                {
                  name: "1008 SQFT",
                  image: "/assets/prakriti/C_408.jpg",
                },
                {
                  name: "1023 SQFT",
                  image: "/assets/prakriti/C_409.jpg",
                },
                {
                  name: "598 SQFT",
                  image: "/assets/prakriti/E_408a.jpg",
                },
              ],
            },
          ],
        },
        subBannerImage: "/league-one/strip-image-the-league-one.webp",
        gallery: {
          heading: "Glimpses of life at prakarthi",
          // subHeading:
          // "With beautiful and thoughtfully-designed homes at kattupakkam, you can experience the sunny side of life.",
          list: [
            {
              name: "Actual Image",
              images: [
                {
                  image: "/league-one/gallery/Block A View_01a.jpg",
                  label: "Project Image",
                },
                {
                  image: "/league-one/gallery/Block A View_02a.jpg",
                  label: "Project Image",
                },
                {
                  image: "/league-one/gallery/Block B View_01a.jpg",
                  label: "Project Image",
                },
                {
                  image: "/league-one/gallery/Block C View_01a.jpg",
                  label: "Project Image",
                },
                {
                  image: "/league-one/gallery/Block C View_02a.jpg",
                  label: "Project Image",
                },
              ],
            },
            {
              name: "Project Image",
              images: [
                {
                  image: "/league-one/gallery/view 01.jpg",
                  label: "Project Image",
                },
                {
                  image: "/league-one/gallery/view 02.jpg",
                  label: "Project Image",
                },
                {
                  image: "/league-one/gallery/view 03.jpg",
                  label: "Project Image",
                },
                {
                  image: "/league-one/gallery/view 04.jpg",
                  label: "Project Image",
                },
              ],
            },
          ],
        },

        testimonial: [
            {
              name: 'Mr. Srinivasan',
              location: '',
              property: 'LML League One',
              description: '',
              image: '',
              alt: 'Mr. Srinivasan testimony',
              video: 'https://www.youtube.com/embed/v5jSn23WuUk?si=4Fcv4lklhuDW97ZB'
          },
          {
              name: 'Mr. Jagadeesh',
              location: '',
              property: 'LML League One',
              description: '',
              image: '',
              alt: 'Mr. Jagadeesh testimony',
              video: 'https://www.youtube.com/embed/us2IfHMZxNE?si=W7sFnD6GWXdhJIF9'
          },
          {
              name: 'Mr. Arun',
              location: '',
              property: 'LML League One',
              description: '',
              image: '',
              alt: 'Mr. Arun testimony',
              video: 'https://www.youtube.com/embed/SeAS5bI1vII?si=xE4HH2fHDsbJkPT7'
          }
        ],
        specifications: [
          {
            name: "structure",
            image: "structure.jpg",
            specifications: [
              {
                pointOne:
                  "RCC framed structure with pile foundation designed to withstand seismic loads as per structural design, conforming to BIS.",
                 pointTwo: "Walls constructed using solid blocks/AAC blocks in cement mortar/adhesive with adequate reinforcement at required levels.",
                 pointThree: "Anti termite treatments wherever applicable during construction stage",
              },
            ],
          },
          {
            name: "Flooring",
            image: "Floor.jpg",
            specifications: [
              {
                pointOne:
                  "Living, dining, and bedroom areas finished with high-quality 2x2 vitrified tiles of the builder's choice, including necessary skirting.",
                 pointTwo: "Toilets, Balcony’s, And utility Area – antiskid ceramic tiles with spacers.",
              },
            ],
          },
          {
            name: "Plumbing & Sanitary",
            image: "Pluming-l1.jpg",
            specifications: [
              {
                pointOne: "Concealed/open plumbing line from UG sump provided.",
                pointTwo: "All closets and washbasins will be white by Jaguar/equivalent or as per the builder's choice, with concealed flush tanks.",
                pointThree: "Provision in plumbing for connecting one geyser in each toilet.",
                pointFour: "Single lever HI – FLOW concealed diverter of by Jaguar / equivalent makes of Hot and cold mixer with overhead shower will be provided in the toilets.",
                pointFive: "Hand shower - provided in all bathrooms.",
                pointSix: "All C.P fittings will be of by Jaguar/equivalent or as per the builder's choice",
                pointSeven: "Toilet closets will be EWC floor/wall mount by Jaguar/equivalent with high quality concealed flush tank cistern of Jaguar/equivalent as per the builder's choice.",
              },
            ],
          },
          {
            name: "Doors",
            image: "Door.jpg",
            specifications: [
              {
                pointOne:
                  "Main Door – (3’6” x 8’0”) 8 Feet Height, frame made of engineered wood with both side veneered flush door (38mm) with necessary SS finished door fittings along with Godrej digital lock / equivalent.",
                 pointTwo: "Bedroom Doors – (3” x 7’0”) 7 Feet Height, frame made of engineered wood with both sides laminated flush door (32mm) with necessary hardware of Godrej / equivalent.",
                 pointThree: "Toilet Doors – (2’6” x 7’0”) 7 Feet Height, frame made of engineered wood with both sides laminated flush door (30mm) with necessary hardware of Godrej / equivalent.",
              },
            ],
          },
          {
            name: "Windows & Ventilators",
            image: "Window_.jpg",
            specifications: [
              {
                pointOne: "Windows and ventilators made of UPVC/aluminium with clear glass panes.",
                pointTwo: "Window Grills will not be provided",
              },
            ],
          },
          {
            name: "Kitchen",
            image: "Kitchen.jpg",
            specifications: [
              {
                pointOne:
                  "Kitchen platform finished with polished granite and fitted with a single bowl stainless steel sink.",
                pointTwo: "Glazed tile dado of 2 feet above the kitchen platform and 3 feet dado in the utility area (if provided).",
                pointThree: "One side Loft will be provided.",
              },
            ],
          },
          {
            name: "Painting",
            image: "Painting.jpg",
            specifications: [
              {
                pointOne: "Living, dining, and bedroom walls finished with two coats of putty and one coat of primer.",
                pointTwo: "Ceiling – 2 coats of smooth putty and 1 coat of primer",
                pointThree: "Bathroom walls covered with ceramic tiles up to ceiling height, as per the builder's choice.",
                pointFour: "Toilet ceiling – Grid type False ceiling",
              },
            ],
          },
          {
            name: "Electrical",
            image: "Electricity.jpg",
            specifications: [
              {
                pointOne: "Concealed copper wiring suitable for three-phase supply, ISI quality.",
                pointTwo: "Switches and sockets with adequate points for lights, fans, and power supply",
                pointThree: "USB charging port in switch boards will be provide in living and in bedrooms.",
                pointFour: "Television points provision in Living room & in master bedroom.",
                pointFive: "Telephone point provision in living room & master bedroom",
                pointSix: "Data point provision in living room & all bedrooms.",
                pointSeven: "Provision for split air conditioner in living room and in all bedrooms.",
                pointEight: "Geyser point will be provided in all the bathrooms.",
                pointNine: "Exhaust fan point will be provided in all bathrooms.",
                pointTen: "Electrical Point for chimney and water purifier.",
                pointEleven: "One calling bell point provided",
                pointTwelve: "Separate meter for common area lighting and pumps.",
                pointThirteen: "Three-phase supply with auto phase changer and trip switch for each flat/apartment.",
                pointFourteen: "Power backup - up to 400w for 2 BHK and 500w for 3 BHK unit.",
              },
            ],
          },
          {
            name: "Common Features",
            image: "Common-features.jpg",
            specifications: [
              {
                pointOne: "Video Intercom System (accessed entry) for each unit from Stilt floor lobby in respective blocks",
                pointTwo: "Fully automatic elevators.",
                pointThree: "100% power backup for the multi-purpose hall, lifts, WTP, STP, and selective common area lighting, with additional backup for other common areas in club house (excluding air conditioning).",
                pointFour: "Lift Fascia – Grantie/equivalent cladding at all levels.",
                pointFive: "Lobby – Granite flooring on Ground floor and tile flooring on other levels.",
                pointSix: "Corridor – vitrified tile floorings at all levels.",
                pointSeven: "Staircase floor – Granite Floorings at all levels",
                pointEight: "Staircase hand rail – SS hand rail at all levels",
                pointNine: "Terrace Floor – Press Tile flooring / cool roof tile.",
                pointTen: "Terrace Door – Good quality door frame with FRP shutter painted on both sides",
                pointEleven: "Name Board – apartment owner’s name will be provided in the ground floor.",
              },
            ],
          },
          {
            name: "Outdoor Features",
            image: "Car-parking.jpg",
            specifications: [
              {
                pointOne: "Borewell and RCC common sump of adequate capacity.",
                pointTwo: "Overhead tank provided for water supply for each block.",
                pointThree: "Rain Water Harvest – Rain water Harvesting site as per site condition.",
                pointFour: "Sewage treatment plant.",
                pointFive: "Water treatment plant.",
                pointSix: "CCTV surveillance across the community.",
                pointSeven: "Car Parking – Grano / tile floorings and driveway(external) – premium quality interlocking pavers / tile flooring as per architect design",
                pointEight: "Security - Security booth will be provided at the entrance gate only with MYGATE App.",
                pointNine: "Exterior Wall – Exterior walls of the building finished with 1 coat of primer & 2 coat of exterior emulsion paint with colour as per architect design.",
                pointTen: "Compound wall on all sides of the property as per architect design",
              },
            ],
          },
        ],
      },
    },
  ],
  upcomingProjects: [
    {
      slug: "PRAKRITI3",
      name: "PRAKRITI",
      color: "#FFFFF",
      location: "Tambaram",
      floor: "2BHK | 3BHK",
      description: "Prakriti is a luxurious residential complex featuring four thoughtfully designed",
      locationInfo: {
        lat: 12.9249,
        lng: 80.1,
      },
      image: "local image file address",
      detail: {
        bannerImage: "bg-home-1.jpeg",
        logo: "logo-1.svg",
        heading: "LML PRAKRITI, TAMBARAM",
        detailDescription:
          "Prakriti in Sanskrit means 'Nature'. It is also what is found in nature in its unaltered form and everything that you can sense. Prakriti is in your thoughts, memories, desires, and even in your aspirations. Bringing Prakriti into your life is LML HOMES, which is an embodiment of one of the finest",
        offer: {
          price: "65 Lakhs",
          info: "2&3 BHK",
        },
        pdf: "assets/pdfs/lml-prakriti-magazine.pdf",
        enquire: {
          whatsApp: "+918898989898",
          phone: "+918898989898",
        },
        bannerHighlights: [
          {
            icon: "",
            label: "2&3 BHK",
          },
          {
            icon: "",
            label: "GATED COMMUNITY",
          },
          {
            icon: "amenities.png",
            label: "2&3 BHK",
          },
          {
            icon: "",
            label: "2&3 BHK",
          },
          {
            icon: "credaui-white.png",
            label: "",
          },
        ],
        descriptionHighlights: [
          {
            icon: "proj-high-2.svg",
            label: "2&3 BHK",
          },
          {
            icon: "proj-high-3.svg",
            label: "GATED COMMUNITY",
          },
          {
            icon: "proj-high-4.svg",
            label: "2&3 BHK",
          },
          {
            icon: "proj-high-5.svg",
            label: "2&3 BHK",
          },
          {
            icon: "proj-high-6.svg",
            label: "2&3 BHK",
          },
          {
            icon: "proj-high-2.svg",
            label: "2&3 BHK",
          },
          {
            icon: "proj-high-3.svg",
            label: "2&3 BHK",
          },
        ],
        walkthrough: {
          video: "video1.webm",
          description:
            "At LML Prakriti, every day offers a new opportunity to immerse yourself in an extravagant lifestyle.",
          question: "WHY LML PRARITI?",
          answer:
            "Naturally set in an enchanting destination and advantageously located at just 2 kms to the Tambaram Railway Station and the Tambaram Main Bus Terminus. Travelling to different parts of the city from Prakriti gets easier and faster thanks to the double connectivity by rail and by road too.",
        },
        amenities: {
          description:
            "At Prakriti, we welcome you to a world of state-of-the-art amenities and facilities that help you savour a premium lifestyle.",
          amenityList: [
            {
              image: "assets/prakriti/gym.png",
              label: "GYM",
            },
            {
              image: "cctv.png",
              label: "CCTV Surviellance",
            },
            {
              image: "Department-Store.png",
              label: "Departmental Store",
            },
            {
              image: "Kids-Play-Area.png",
              label: "Kids Play Area",
            },
            {
              image: "lift.png",
              label: "Elevator",
            },
            {
              image: "Mini-Hall.png",
              label: "Mini Hall",
            },
            {
              image: "Mini-Theatre.png",
              label: "Mini Theatre",
            },
            {
              image: "Parking.png",
              label: "Parking",
            },
            {
              image: "security.png",
              label: "24x7 Security",
            },
            {
              image: "Sewage-Treatment-Plant.png",
              label: "Sewage Treatment Plant",
            },
            {
              image: "Temple.png",
              label: "Temple",
            },
            {
              image: "Walking-Path-way.png",
              label: "Walking-Path-way",
            },
          ],
        },
        locationInfo: {
          heading: "Location Tambaram",
          description:
            "This gated community apartments is the perfect place to find a home that's conveniently close to all the key infrastructures you need. With just a 5-minute drive, you can reach college, office, hospitals, malls and more.",
          locations: [
            {
              name: "Commute",
              icon: "commute.svg",
              locationList: [
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "Education",
              icon: "education.svg",
              locationList: [
                {
                  name: "Chilrens Academy Education - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
          ],
        },
        layout: {
          heading: "LAYOUTS $ PLANS",
          description:
            "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "Block 1 First Floor",
              image: "block-1.png",
            },
            {
              name: "Block 1 First Floor",
              image: "block-1.png",
            },
            {
              name: "Block 1 First Floor",
              image: "block-1.png",
            },
          ],
        },
        unitPlan: {
          heading: "Unit Plans",
          // description:
          //   "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
              ],
            },
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
              ],
            },
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
              ],
            },
          ],
        },
        subBannerImage: "adobe-of-nature.png",
        gallery: {
          heading: "Glimpses of life at prakarthi",
          // subHeading:
          //   "With beautiful and thoughtfully-designed homes at Tambaram, you can experience the sunny side of life.",
          list: [
            {
              name: "Project Size",
              images: [
                {
                  image: "project-carousel-img.png",
                  label: "ACTUAL FRONT ELEVATION VIEW",
                },
                {
                  image: "project-carousel-img.png",
                  label: "ACTUAL FRONT ELEVATION VIEW",
                },
                {
                  image: "project-carousel-img.png",
                  label: "ACTUAL FRONT ELEVATION VIEW",
                },
              ],
            },
          ],
        },
      },
    },
  ],
  completedProjects: [
    {
      slug: "PRAKRITI4",
      name: "PRAKRITI",
      color: "#FFFFF",
      location: "Tambaram",
      floor: "2BHK | 3BHK",
      description: "Prakriti is a luxurious residential complex featuring four thoughtfully designed",
      locationInfo: {
        lat: 12.9249,
        lng: 80.1,
      },
      image: "local image file address",
      detail: {
        bannerImage: "bg-home-1.jpeg",
        logo: "logo-1.svg",
        heading: "LML PRAKRITI, TAMBARAM",
        detailDescription:
          "Prakriti' in Sanskrit means 'Nature'. It is also what is found in nature in its unaltered form and everything that you can sense. Prakriti is in your thoughts, memories, desires, and even in your aspirations. Bringing Prakriti into your life is LML HOMES, which is an embodiment of one of the finest",
        offer: {
          price: "65 Lakhs",
          info: "2&3 BHK",
        },
        pdf: "assets/pdfs/lml-prakriti-magazine.pdf",
        enquire: {
          whatsApp: "+918898989898",
          phone: "+918898989898",
        },
        bannerHighlights: [
          {
            icon: "",
            label: "2&3 BHK",
          },
          {
            icon: "",
            label: "GATED COMMUNITY",
          },
          {
            icon: "amenities.png",
            label: "2&3 BHK",
          },
          {
            icon: "",
            label: "2&3 BHK",
          },
          {
            icon: "credaui-white.png",
            label: "",
          },
        ],
        descriptionHighlights: [
          {
            icon: "proj-high-2.svg",
            label: "2&3 BHK",
          },
          {
            icon: "proj-high-3.svg",
            label: "GATED COMMUNITY",
          },
          {
            icon: "proj-high-4.svg",
            label: "2&3 BHK",
          },
          {
            icon: "proj-high-5.svg",
            label: "2&3 BHK",
          },
          {
            icon: "proj-high-6.svg",
            label: "2&3 BHK",
          },
          {
            icon: "proj-high-2.svg",
            label: "2&3 BHK",
          },
          {
            icon: "proj-high-3.svg",
            label: "2&3 BHK",
          },
        ],
        walkthrough: {
          video: "video1.webm",
          description:
            "At LML Prariti, every day offers a new opportunity to immerse yourself in an extravagant lifestyle.",
          question: "WHY LML PRARITI?",
          answer:
            "Naturally set in an enchanting destination and advantageously located at just 2 kms to the Tambaram Railway Station and the Tambaram Main Bus Terminus. Travelling to different parts of the city from Prakriti gets easier and faster thanks to the double connectivity by rail and by road too.",
        },
        amenities: {
          description:
            "At Prakriti, we welcome you to a world of state-of-the-art amenities and facilities that help you savour a premium lifestyle.",
          amenityList: [
            {
              image: "assets/prakriti/gym.png",
              label: "GYM",
            },
            {
              image: "cctv.png",
              label: "CCTV Surviellance",
            },
            {
              image: "Department-Store.png",
              label: "Departmental Store",
            },
            {
              image: "Kids-Play-Area.png",
              label: "Kids Play Area",
            },
            {
              image: "lift.png",
              label: "Elevator",
            },
            {
              image: "Mini-Hall.png",
              label: "Mini Hall",
            },
            {
              image: "Mini-Theatre.png",
              label: "Mini Theatre",
            },
            {
              image: "Parking.png",
              label: "Parking",
            },
            {
              image: "security.png",
              label: "24x7 Security",
            },
            {
              image: "Sewage-Treatment-Plant.png",
              label: "Sewage Treatment Plant",
            },
            {
              image: "Temple.png",
              label: "Temple",
            },
            {
              image: "Walking-Path-way.png",
              label: "Walking-Path-way",
            },
          ],
        },
        locationInfo: {
          heading: "Location Tambaram",
          description:
            "This gated community apartments is the perfect place to find a home that's conveniently close to all the key infrastructures you need. With just a 5-minute drive, you can reach college, office, hospitals, malls and more.",
          locations: [
            {
              name: "Commute",
              icon: "commute.svg",
              locationList: [
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "Education",
              icon: "education.svg",
              locationList: [
                {
                  name: "Chilrens Academy Education - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
            {
              name: "HealthCare",
              icon: "healthcare.svg",
              locationList: [
                {
                  name: "Chilrens Academy HealthCare - 1.5km",
                  location: {
                    lat: 12.923530127810208,
                    lng: 80.09982833863425,
                  },
                },
                {
                  name: "Chilrens Academy - 1.5km",
                  location: {
                    lat: 12.9249,
                    lng: 80.1,
                  },
                },
              ],
            },
          ],
          address: [{ address: " Sridevi Nagar 3rd street,Goparasanallur, Kattupakkam, Chennai - 600 056" }],
        },
        layout: {
          heading: "LAYOUTS $ PLANS",
          description:
            "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "Block 1 First Floor",
              image: "block-1.png",
            },
            {
              name: "Block 1 First Floor",
              image: "block-1.png",
            },
            {
              name: "Block 1 First Floor",
              image: "block-1.png",
            },
          ],
        },
        unitPlan: {
          heading: "Unit Plans",
          // description:
          //   "Browse through our various floor-plan configurations. Each category of room provides an array of facilities, meticulously designed to cater to your requirements.",
          layoutPlan: [
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
              ],
            },
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
              ],
            },
            {
              name: "2BHK",
              sizeList: [
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
                {
                  name: "1115 SQFT",
                  image: "unitPlans.png",
                },
              ],
            },
          ],
        },
        subBannerImage: "adobe-of-nature.png",
        gallery: {
          heading: "Glimpses of life at prakarthi",
          //subHeading:
          //"With beautiful and thoughtfully-designed homes at Tambaram, you can experience the sunny side of life.",
          list: [
            {
              name: "Project Size",
              images: [
                {
                  image: "project-carousel-img.png",
                  label: "ACTUAL FRONT ELEVATION VIEW",
                },
                {
                  image: "project-carousel-img.png",
                  label: "ACTUAL FRONT ELEVATION VIEW",
                },
                {
                  image: "project-carousel-img.png",
                  label: "ACTUAL FRONT ELEVATION VIEW",
                },
              ],
            },
          ],
        },
      },
    },
  ],
};

export { overAllData };
